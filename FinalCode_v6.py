...
# ==============================================================================
# STEP 12 - REGRESSION (predicting the drowsiness score 0-5)
# ==============================================================================

print("\nTraining Linear Regression to predict drowsiness score (0-5)...")

y_reg = df['Rate your drowsiness'].dropna()
X_reg = df.loc[y_reg.index, selected_features]

X_train_r, X_test_r, y_train_r, y_test_r = train_test_split(
    X_reg, y_reg, test_size=0.2, random_state=24
)
...
print(f"MAE  : {mean_absolute_error(y_test_r, y_pred_r):.3f} points")
print(f"R²   : {r2_score(y_test_r, y_pred_r):.3f}")

# actual vs predicted scatter
plt.figure(figsize=(7, 5))
plt.scatter(y_test_r, y_pred_r, color='#1565C0', edgecolors='black', alpha=0.7, s=60)
plt.plot([0, 5], [0, 5], 'r--', lw=2, label='Perfect Prediction Line')
plt.xlabel('Actual Drowsiness Score', fontsize=12)
plt.ylabel('Predicted Drowsiness Score', fontsize=12)
plt.title(f'Linear Regression — Actual vs Predicted\n'
          f'MAE={mean_absolute_error(y_test_r, y_pred_r):.3f}  '
          f'R²={r2_score(y_test_r, y_pred_r):.3f}',
          fontsize=13, fontweight='bold')
